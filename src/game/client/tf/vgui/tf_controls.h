...
public:
	BlameNames_t( const CUtlVector< CSteamID >& vecBlameSteamIDs, const char* pszReason, const char* pszSingularVerb, const char* pszPluralVerb )
	{
		...
		FOR_EACH_VEC( vecBlameSteamIDs, i )
		{
			if ( i == 0 )
			{
				g_pVGuiLocalize->ConstructString( wszMembers, sizeof( wszMembers ), L"%s1", 1, CStrAutoEncode( steamapicontext->SteamFriends()->GetFriendPersonaName( vecBlameSteamIDs[ i ] ) ).ToWString() );
			}
			else if ( i == vecBlameSteamIDs.Count() - 1 )
			{
				g_pVGuiLocalize->ConstructString( wszMembers, sizeof( wszMembers ), g_pVGuiLocalize->Find( "#TF_PartyMemberState_LastTwo" ), 2, CStrAutoEncode( wszMembers ).ToWString(), CStrAutoEncode( steamapicontext->SteamFriends()->GetFriendPersonaName( vecBlameSteamIDs[ i ] ) ).ToWString() );
			}
			else
			{
				g_pVGuiLocalize->ConstructString( wszMembers, sizeof( wszMembers ), L"%s1, %s2", 2, CStrAutoEncode( wszMembers ).ToWString(), CStrAutoEncode( steamapicontext->SteamFriends()->GetFriendPersonaName( vecBlameSteamIDs[ i ] ) ).ToWString() );
			}
		}

		g_pVGuiLocalize->ConstructString( wszMembers, sizeof( wszMembers ), vecBlameSteamIDs.Count() == 1 ? g_pVGuiLocalize->Find( pszSingularVerb ) : g_pVGuiLocalize->Find( pszPluralVerb ) , 1, CStrAutoEncode( wszMembers ).ToWString() );
		g_pVGuiLocalize->ConstructString( m_wszBuff, sizeof( m_wszBuff ), g_pVGuiLocalize->Find( pszReason ), 1, wszMembers );
	}
