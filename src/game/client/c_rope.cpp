...
void C_RopeKeyframe::BuildRope( RopeSegData_t *pSegmentData, const Vector &vCurrentViewForward, const Vector &vCurrentViewOrigin, C_RopeKeyframe::BuildRopeQueuedData_t *pQueuedData, bool bQueued )
{
	...
	for( int iNode = 0; iNode < nodeCount; ++iNode )
	{
		...
		pSegmentData->m_Segments[nSegmentCount].SetColor( pLightValues[iNode] * vColorMod );
		...
		if ( iNode < lastNode )
		{
			...
			{
				Vector colorAsVector( pSegmentData->m_Segments[nSegmentCount-1].m_color.r, pSegmentData->m_Segments[nSegmentCount-1].m_color.g, pSegmentData->m_Segments[nSegmentCount-1].m_color.b );
				pSegmentData->m_Segments[nSegmentCount].SetColor( colorAsVector + vecColorInc );
	...
	if ( ShouldUseFakeAA( m_pBackMaterial ) )
	{
		...
		for ( int iSegment = 0; iSegment < nSegmentCount; ++iSegment )
		{
			...
			{
				pSegmentData->m_Segments[iSegment].SetAlpha( flMinAlpha );
			...
			else
			{
				...
				{
					pSegmentData->m_Segments[iSegment].SetAlpha( flMaxAlpha );
				}
				else
				{
					pSegmentData->m_Segments[iSegment].SetAlpha( RemapVal( flScreenSpaceWidth, flMinScreenSpaceWidth, flMaxAlphaScreenSpaceWidth, flMinAlpha, flMaxAlpha ) );
				}
	...
	else
	{
		...
		{
			pSegmentData->m_Segments[iSegment].m_flTexCoord = flTexCoord;
			pSegmentData->m_Segments[iSegment].SetAlpha( 0.3f );
			pSegmentData->m_Segments[iSegment].m_flWidth = m_Width;
