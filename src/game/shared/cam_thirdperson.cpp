...
CThirdPersonManager g_ThirdPersonManager[ MAX_SPLITSCREEN_PLAYERS ];

CThirdPersonManager &GetThirdPersonManager( int slot )
{
	if ( slot == -1 )
	{
		ASSERT_LOCAL_PLAYER_RESOLVABLE();
		return g_ThirdPersonManager[ GET_ACTIVE_SPLITSCREEN_SLOT() ];
	}
	return g_ThirdPersonManager[ slot ];
}