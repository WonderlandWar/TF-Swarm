...
{
	CBaseMultiplayerPlayer *pPlayer = static_cast<CBaseMultiplayerPlayer*>( pl );
	if ( pPlayer )
	{
...
void CTeamplayRoundBasedRules::BalanceTeams( bool bRequireSwitcheesToBeDead )
{
	...
	// sort the vector
	//vecGroups.Sort( SwitchPlayersSort );
	...
	for ( int i=vecGroups.FirstInorder(); i != vecGroups.InvalidIndex(); i = vecGroups.NextInorder( i ) )
	{
		...
		{
			pPlayer = static_cast<CBaseMultiplayerPlayer*>( group.GetPlayer( j ) );
			if ( !pPlayer )
				continue;			
